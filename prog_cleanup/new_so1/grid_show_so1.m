...
% Several deviations
if 1
   [~, yMax1] = figures_lh.y_range(devV);
   [~, yMax2] = figures_lh.y_range([0, 2 * min(devV)]);
...
%    if any(iqDevV > 0)  &&  cS.calS.tgIq > 0
%       iLine = iLine + 1;
%       plot(xV, iqDevV, cS.lineStyleV{iLine}, 'color', cS.colorM(iLine,:));
%       legendV{iLine} = 'IQ';
%    end
   
%    if any(devIqExperV > 0)  &&  cS.calS.tgBetaIqExper > 0
%       iLine = iLine + 1;
...
save_fig_so([figDir, 'sp_g_sp'], saveFigures, figS.figOpt4S, cS);
