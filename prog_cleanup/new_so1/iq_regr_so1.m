function coeffV = iq_regr_so1(wage_itscM, pSchool_iscM, iq_icM, cS)
...
IN:
   wage_itscM(ind, age, school, cohort)
...
%% Input check
if cS.dbg > 10
   if ~v_check(wage_itscM, 'f', [cS.nSim, cS.demogS.ageRetire, cS.nSchool, cS.nCohorts], 1e-3, 1e3, cS.missVal)
      error_so1('Invalid');
   end
...
%% Make regressors
% Same dimensions as wage_itscM

logWage_itscM = log_lh(wage_itscM(:,:,:,iCohortV), cS.missVal);

sizeV = size(logWage_itscM);
experM = zeros(sizeV);
yearM  = zeros(sizeV);
...
validM = (experM >= 1)  &  (yearM > 1900)  &  (yearM <= maxYear)  &  (logWage_itscM ~= cS.missVal);
vIdxV = find(validM == 1);
...
rsS = lsq_weighted_lh(logWage_itscM(vIdxV), xM, wtM(vIdxV), cS.rAlpha, cS.dbg);
