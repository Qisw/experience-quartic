...
outS.lSupply_stM = calibr_so1.aggr_ls(simS.lPerHour_tscM,  tgS.aggrHours_tsyM, cS);
...
% Log wage profiles
[scalarMeanDev, devS.ssd_scM] = calibr_so1.dev_wage_profiles(simS.logWage_tscM, ...
   tgS.logWage_tscM, tgS.wageWt_tscM, cS);
...
% %%  Penalty if ability sorting is not monotone
% 
% if 1
%    entryDiffM = diff(simS.collEntryAbilGroup_acM, 1, 1);
%    if any(entryDiffM(:) < 0)
%       sortingPenalty = sum(max(0, -entryDiffM(:))) .* 10;
%       outS.devVector = outS.devVector.add('a sorting', sortingPenalty);
%    end
% end
